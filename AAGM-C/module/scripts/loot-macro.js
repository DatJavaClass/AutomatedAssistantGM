// Loot Watchdog macro source, serialized by bridge.js.
...
async function lootWatchdogMain() {
  ...
  // Every journal write serializes through one promise chain
  ...
  // Phantom check: physical type plus renderable subType only
  ...
  // Duck-type Item Piles args; source before target
  ...
  const snapshotFromEntry = (entry, sourceActor) => {
    ...
    if (!data) return null;
    // Post-hook deltas may be signed; only magnitude matters
  ...
  // Arrivals can race the record; avoids false vanish
  ...
  function track(source, target, entry, userId, via) {
    ...
    // Own transfers arrive twice; one record dedupes both
  ...
  // Pre hooks fire only on this client
