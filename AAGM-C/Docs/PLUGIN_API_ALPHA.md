...
pastes it into their world; Claude invokes it through the bridge's `eval`,
and every mutating call takes a **Rollback Point** before it runs (or renders
an **Approve/Deny card** under the legacy confirm policy). Any member of the
AAGM family can call a plug-in; the contract does not care which model. You get the whole Foundry API on the GM client; the
