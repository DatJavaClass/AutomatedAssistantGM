...
## 15. 2.0: interrupts, Rollback Points, seats, the day log (built 2026-09-24)

Origin: DatJavaClass, 2026-09-24. Four asks from live use:
(1) prompt lag: while Claude works on Foo, a Foo+ addendum had to wait for
the next poll; (2) the confirmation gates were sound in theory and slowed
everything down in practice, so Rollback Points replace them as the
default write path; (3) `/ext` and `/int` declare which seat he is in (a
terminal Claude session versus the box); (4) log everything to a plain day
file. §9 and §13.3 are not deleted: they live on under the `confirm` write
policy.

### 15.1 Interrupts

- A prompt typed on a tab whose state is `working` or `gated` is queued with
  `interrupt:true` (`relay/src/prompt-queue.js`).
- Delivered once, whichever comes first: piggybacked on a `foundry_*` tool
  result as `interrupts` (plus an `interruptNote`), or the next
  `foundry_get_prompts`. `promptQueue.takeInterrupts(tabId)` hands over
  that tab's interrupts when the tool call names a tab; with no `tabId` it
  hands over any tab's interrupts only when `multitasking` is off (with
  tabs on, an untabbed call carries none).
- Every tool result can carry them except `foundry_get_prompts` (which
  returns them as prompts anyway), `foundry_log` and `foundry_surface`.
- Relay authored prompts (`system:true`, the rollback notice in 15.2) use
  the same lane: they are queued with `interrupt:true` too.
- The box marks an interrupt itself (its tab is working or gated and the
  text is not a relay command) with a lightning glyph and "(interrupt)" on
  the speaker label.

### 15.2 Rollback Points

- **Policy.** World setting `writePolicy`: `rollback` (default) or
  `confirm` (legacy §9 gate plus Chain Mode). Independent of the mode
  presets; the presets never touch it. The relay coerces anything other
  than `confirm` to `rollback` (`relay/src/world-settings.js`).
- **Write path under rollback.** The relay opens a point
  (`rp-<6 hex>`) and sends the write request with `params.rp = {id}`. The
  module (`bridge.js`) runs the handler inside `runRecorded`
  (`module/scripts/rollback-recorder.js`): one recording at a time,
  serialized on a promise chain. Hooks on every document type
  (`preUpdate`, `preCreate` plus `create`, `preDelete`) capture the before
  state of updates, the uuids of creates (tagged through
  `options._aagmRp`), and the full data of deletes (with `parentUuid` and
  `pack`). Earliest state wins: a document is snapshotted once per point,
  and an update to a document created in the same point is not recorded.
  A snapshot over 4,000,000 characters is stored as null and marked
  `oversized`; a document whose `toObject()` throws is marked `partial`
  (only `_id` and name kept). Both flags live on the entry only: nothing
  warns at write time, and an oversized entry fails at rollback.
- **Return.** The module returns `rollback.entries` with the result, or in
  the JSON RPC error data when the handler throws (writes made before the
  throw are still captured). The relay persists
  `relay/rollbacks/<date>/<id>.json`, keeps an index of up to 200 points
  (today's reload on restart), pushes `claude.rollback {event:'point'}`,
  writes a `write` line to the day log, and hands `rollbackPoint` back to
  Claude.
- **Rolling back.** Rolling back to a point restores it and every live
  point after it, newest first, one `rollback.apply` request per point.
  Inverse ops (`module/scripts/handlers/rollback.js`, run exclusive with
  recordings): update = `d.update(before, {diff:false, recursive:false})`
  with `_id`, `_stats`, `type` and the embedded collections stripped;
  create = delete (already gone counts as ok); delete = recreate with
  `keepId`, embedded ones through `parent.createEmbeddedDocuments`, top
  level ones into their pack when they had one. Outcome per point:
  `live`, `rolled-back`, or `partial` (some entry failed). A bridge error
  stops the chain. One rollback at a time (`rollback-busy`).
- **Triggers.** The box card button (`claude.rollback.request`), a typed
  `/rollback [id]` (alias `/rb`), or `foundry_rollback`. Omitting the id
  means the latest live point. A rollback the GM triggers (card or typed)
  that restores at least one point reaches the loop as a system prompt
  (`system:true`, `rollback:true`) telling it to reread the world; one
  Claude triggers through `foundry_rollback` sends no prompt (it already
  holds the result).
- **Chain Mode** is refused under rollback (`foundry_chain_offer` returns
  `{refused}`): there are no gates to chain.
- **Damage.** `foundry_apply_damage` still computes the plan preview; a
  lethal plan is flagged in the point summary and the result, and the
  commit applies at once with `allowLethal:true`.
- **Limits.** Documents only: no files, no writes made in another window,
  writes the code does not await may escape the recording, and the GM's own
  writes in the bridge window during a recording land in the point. No
  redo: a rollback cannot be rolled back.

### 15.3 Seats

- `/ext` and `/int` typed in the box, or `foundry_surface {mode}`.
- `ext` parks the box: prompts typed there are held in `parked` (the box
  gets a "Parked until /int" line) and released into the queue on `/int`.
  `claude.status` reports state `ext`; `surface` rides `foundry_ping` and
  `foundry_get_prompts`.
- Under `ext` the loop idles except the loot sweep.
- An AAGM Advanced session declares `ext` when it starts driving and `int`
  when it hands the seat back.

### 15.4 Day log

- `Logs/YYYY-MM-DD.md` at the project root (`config.logDir` overrides,
  relative to `relay/`). Rollback snapshots go to `config.rollbackDir`,
  default `relay/rollbacks`. Local clock (`relay/src/daylog.js`).
- Each file opens with `# AAGM-C log <date>`; lines are
  `- HH:MM:SS [tab] kind: text` (the tag reads `[<tabId> ext]` while the
  external seat is active; text collapsed to one line, capped at 600
  characters).
- Kinds: `prompt`, `interrupt`, `reply`, `read`, `write`, `write-failed`,
  `denied`, `blocked`, `damage`, `rollback`, `seat`, `loot`, `session`,
  `relay`. Under the rollback policy damage logs as `write` (through its
  point); `damage` is the confirm policy line.
- Read with `foundry_log {date?, tail?, list?}` or `/log [n]` in the box
  (default 20 lines). `/points` (alias `/rps`) lists today's points.

### 15.5 Protocol delta

| Message | Direction | Change |
|---|---|---|
| `claude.rollback` | relay → module | New. `event`: `point` (new point), `rolled-back`, `sync` (all points, sent on `claude.hello`), `error`. |
| `claude.rollback.request` `{pointId}` | module → relay | New. The box card's Roll back button. |
| `rollback.apply` `{pointId, entries}` | relay → module (request) | New handler, in the `debug` capability set. Returns `{pointId, applied:[{uuid,op,ok,error?}]}`. |
| `eval` / `damage` params | relay → module | Gain `rp: {id}`; the result gains `rollback.entries` (error data too). |
| `claude.status` | relay → module | Gains `surface`; new state `ext`. |
| `claude.reply` | relay → module | Gains `system` (relay authored line). |
| tab transcript | relay | New role `sys` for relay authored lines. |

Tool delta: 18 tools. New `foundry_rollback_points`, `foundry_rollback`,
`foundry_log`, `foundry_surface`. `foundry_eval` (write or destructive)
and `foundry_apply_damage` return `rollbackPoint` under the rollback
policy. Results may carry `interrupts` (15.1). `foundry_ping` gains
`surface`, `rollbackPoints` and `logFile`.

### 15.6 Loop behavior (`/aagm` skill)

- An `interrupts` array on any tool result is read at once: fold it into
  the current task (or stop if it says so), acknowledge in the box with
  `foundry_send_reply final:false`, continue.
- A `system:true` prompt is a rollback notice: reread the world before
  continuing.
- Writes carry an honest `intent` and `summary` (the summary labels the
  point), and every write is followed by a separate read back. When the
  read back shows the write went wrong, `foundry_rollback` it.
- `surface` `ext`: do no work, keep polling and sweeping loot until it
  reads `int` again.

### 15.7 Box UI

- Every point renders as a card in its tab: id, kind, summary, captured
  counts, a collapsible document list (first 12), and a Roll back button
  while live. Cards dedupe by point id (`sync` resends them on hello).
- A rolled back card loses its button and gains "Rolled back (via ...)"
  or "Partially rolled back, check the day log"; a `sys` line repeats it.
- Interrupts get the lightning glyph label (15.1). Under `ext` the status
  line reads the external seat message.
- The settings menu gains the `writePolicy` radio pair.

### 15.8 Build order

All built 2026-09-24; deploy pending the live test.

1. Relay: `daylog.js`, `rollback.js`, interrupts and seats in
   `prompt-queue.js`, `writePolicy` in `world-settings.js`, four new tools
   in `mcp-server.js`.
2. Module: `rollback-recorder.js`, `handlers/rollback.js`, `rp` wrapping in
   `bridge.js`, the `writePolicy` setting.
3. Box: point cards, interrupt glyph, `ext` status.
4. Skills: `/aagm` and AAGM Advanced.
5. Docs: this section, PHASE1_STATUS, CLAUDE.md.
6. Stamp 2.0.0, rebuild the zip, Dropbox in place byte write.
