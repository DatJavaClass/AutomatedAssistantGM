// WebSocket server for the bridge module.

import { WebSocketServer } from 'ws';
import { randomUUID } from 'node:crypto';

// Capability sets: config.json maps users; DESIGN §5.1 truth
const CAPABILITY_SETS = {
  debug: new Set([
    ...
    'logs.subscribe',
    'logs.unsubscribe',
    // eval: reads free, writes gated, DB-journal refused
    'eval',
    // damage: constrained damage primitive, §9-gated; a below-1-HP outcome
    // escalates to a double confirm.
    'damage',
    // Loot Watchdog queue: pending reads, restore recreates ungated
    'loot.pending',
    'loot.restore',
    // 2.0 Rollback Points: restore a recorded snapshot (handlers/rollback.js).
    'rollback.apply',
  ]),
...
function handleConnection(socket, req, { config, dispatcher, audit, worldSettings }) {
  ...
  // Defense in depth: refuse non-loopback if bind bypassed
  ...
  socket.on('message', (raw) => {
    ...
    if (!helloSeen) {
      ...
      // Register with dispatcher: validated metadata plus send closure
      ...
      // §13.2: hello carries settings; later sync via notifications
    ...
    // Post-hello: our response, or a bridge notification only
...
function validateHello(msg, { config, dispatcher, audit }) {
  ...
  // Refuse a second connection for the same userId
