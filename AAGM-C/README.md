...
*Foundry ↔ Claude bridge. Formerly plain "AAGM". The -C marks the Claude member of the family; the siblings for Codex, Grok, and Kimi live one folder up, see [the family README](../README.md).*

**Jump to:** [Installation](#installation) · [Setup](#a-setup-one-time) · [Using it](#b-using-it-once-deployed) · [Troubleshooting](#troubleshooting-quick-hits) · [The family](#the-family) · [The Plug-in API](#the-plug-in-api) · [License](#license)
...
```
https://raw.githubusercontent.com/DatJavaClass/AutomatedAssistantGM/grape/AAGM-C/module/module.json
```
That is the 2.0 manifest for this folder; each sibling folder carries its own. You'll enable the module later (see Setup → running a session).
...
- **Eval:** `foundry_eval` runs JS in the GM client. Reads run freely; mutating/destructive code is reclassified at the relay and, under the default rollback policy, runs at once behind a Rollback Point (the confirm policy routes it through the old gate instead). DB-backing journals are hard-refused.
- **Damage:** `foundry_apply_damage` previews before→after on live HP, flags a lethal outcome, and applies with a Rollback Point (confirm policy: single confirm, lethal escalates to a **double** confirm). Healing or setting HP directly goes through `foundry_eval`.
- **Rollback:** `foundry_rollback_points` lists today's points; `foundry_rollback` restores to before one of them (that point and every live point after it, newest first).
- **Log and seat:** `foundry_log` reads the day file; `foundry_surface` reads or sets `int`/`ext`.
- **Chat channel:** `foundry_get_prompts` / `foundry_send_reply`. Tab aware since 0.9.0: every prompt carries a `tabId`, every reply echoes it. Any tool result may carry `interrupts`: messages typed mid task.
- **Loot rescue:** `foundry_loot_pending` / `foundry_restore_loot` (see the Claude Loot Watchdog below).
- **Chain Mode:** `foundry_chain_offer` - one approval covering a declared batch of same-shaped writes (confirm policy only; refused under rollback).

### Two GUI surfaces (auto-created macros in Foundry)
Both appear in your macro directory once the bridge connects:
- **"Open Claude Code Chat"** - the in-Foundry chat box. To use it: in Claude Code run a tight loop that calls `foundry_get_prompts` (it long-polls ≤25s) and answers with `foundry_send_reply`, e.g. `/loop 2s` instructed to call `foundry_get_prompts` back-to-back. Open the macro: it shows "Ready to chat" once the loop polls; type → Claude answers in the box. Every write renders a **Rollback point** card with a Roll back button (under the confirm policy it is the old **Approve/Deny** card instead, deletes needing a **double** confirm). Typing while Claude is working marks the message as an interrupt and it reaches Claude mid task. Box commands: `/ext` and `/int`, `/log [n]`, `/rollback [id]`, `/points`, `/close`, `/exit`. In Co-GM Mode the box has **tabs**: one tab is one task, up to five, each served by its own background agent. Close a tab (the x, or type `/close` in it) and its agent stops.
...
### Pick your mode
*Configure Settings → AAGM-C Settings* (GM only). **Assistant Mode** is the default: every change individually confirmed. **Co-GM Mode** trusts you with multitasking (the tab bar) and Chain Mode offers. **Custom** unlocks the individual switches, including the local **Macro Mirror** (that one is available in every mode). The **Write policy** sits beside the modes: Rollback Points (2.0 default) or Confirmation gates (legacy).

### Stop
- End the chat loop: type `/exit` in the box, or `touch relay/.loop-stop`.
- Stop the relay: `Ctrl+C`.

### Safety net (do not bypass)
Rollback Point on every write (the module snapshots every document a write touches before it runs) · honest intent and a plain summary on every write, plus a separate read back · DB-journal access refused · a rollback never writes a linked token's delta through to its actor · one chat-box listener at a time · relay binds localhost only. The confirmation gate (all writes, double confirm on deletes and lethal damage, Chain Mode) is still there as the `confirm` write policy. These are load-bearing - never weaken them. Rollback covers Foundry documents only: uploaded files and things done in another window are not in the snapshot.
...
AAGM-C is one of four. The siblings connect the same kind of bridge to Codex ([`AAGM-O/`](../AAGM-O)), Grok ([`AAGM-G/`](../AAGM-G)), and Kimi ([`AAGM-K/`](../AAGM-K)); each folder has its own README, manifest, and listener prompt. The [family README](../README.md) has the table.

---

## The Plug-in API

The bridge is extensible, and the four plug-ins below are the proof: each one is just a world macro serving as an endpoint behind the gated eval - scope object in, `{ ok }` / `{ ok:false, error }` out, bare run as a diagnostic. The alpha contract is written up in [`Docs/PLUGIN_API_ALPHA.md`](Docs/PLUGIN_API_ALPHA.md); build your own and it rides the same Rollback Point as everything else (or the Approve/Deny gate under the confirm policy).
...
**2.0.0 - Rollback Points (2026-09-24)**
- Four things changed, all of them because the 0.x way was slower than the GM.
- **Interrupts.** Type while Claude is mid task and the message lands in its very next tool call, not after the task finishes. The box marks it with a lightning bolt so you know it registered.
- **Rollback Points.** The Approve/Deny gates are gone by default. Every write snapshots what it touches first (hooks on every document type, packs included), and a Roll back button in the box, or `/rollback [id]`, puts the world back the way it was. Rolling back an older point rolls back everything after it too. The gates still exist as a write policy setting if you want them back.
- **/ext and /int.** Tell the box you have moved to a terminal Claude session and it parks itself until you come back. Parked messages wait, they do not vanish.
- **A log.** One markdown file per day under `Logs/`, one line per thing done. No fancy name, no fancy anything. `/log` shows the tail, `foundry_log` reads it.
- Under the hood: four new tools (`foundry_rollback_points`, `foundry_rollback`, `foundry_log`, `foundry_surface`), `rollbackPoint` on every write result, `interrupts` on any result, `writePolicy` in the settings menu. The family moved into one repo with a folder per member on the same day.
