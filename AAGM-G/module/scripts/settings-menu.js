...
const L = (k) => game.i18n.localize(`AAGM_G.SETTINGS.${k}`);
...
export class AagmSettingsMenu extends FormApplication {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      id: 'aagm-settings',
      title: game.i18n.localize('AAGM_G.SETTINGS.MenuTitle'),
  ...
  async _renderInner() {
    ...
    return $(`<form autocomplete="off">
        ...
        <fieldset data-aagmg="opts"><legend>${L('OptsLegend')}</legend>
  ...
  activateListeners(html) {
    ...
    html.find('input[name="mode"]').on('change', (ev) => {
      ...
      html.find('[data-aagmg="opts"] input').prop('disabled', !custom);
  ...
  async _updateObject(_event, formData) {
    ...
    ui.notifications?.info(game.i18n.localize('AAGM_G.SETTINGS.Saved'));
  }
}
