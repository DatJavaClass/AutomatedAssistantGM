/* "Open AAGM-G Chat" macro source, serialized via .toString().
   Self-contained: runtime globals + module api only. */

async function chatBoxMain() {
  const MODULE_ID = 'aagm-g';
  const STYLE_ID = 'aagmg-chat-theme';
  const LAYOUT_ID = 'aagmg-chat-layout';
  const L = (k) => game.i18n.localize('AAGM_G.CHAT.' + k);
  ...
  if (!document.getElementById(LAYOUT_ID)) {
    ...
    s.textContent = `
      .aagmg-wrap { display:flex; flex-direction:column; gap:8px; }
      .aagmg-status { font-size:12px; padding:5px 8px; border-radius:3px; border:1px solid #00ffcc; }
      .aagmg-status.ready { color:#00ffcc; border-color:#00ffcc; }
      .aagmg-status.warn  { color:#ffaa00; border-color:#ffaa00; }
      .aagmg-tabs { display:flex; align-items:stretch; gap:2px; min-height:28px; overflow-x:auto; }
      .aagmg-tab { width:auto; height:auto; line-height:1.2; flex:1 1 0; min-width:0; display:flex;
      ...
      .aagmg-tab.aagmg-active { background:#141414; color:#fff; border-color:#00ffcc; }
      .aagmg-tab::before { content:''; flex:none; width:6px; height:6px; border-radius:50%; background:#777; }
      .aagmg-tab.aagmg-working::before { background:#00ffcc; }
      .aagmg-tab.aagmg-done::before { background:#66cc66; }
      .aagmg-tab-t { flex:1 1 auto; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
      .aagmg-tab-x { opacity:0.6; padding:0 2px; flex:none; }
      .aagmg-tab-x:hover { opacity:1; color:#ff6666; }
      .aagmg-tab-add { width:auto; height:auto; line-height:1.2; flex:none; padding:4px 9px;
      ...
      .aagmg-tab-add:disabled { opacity:0.35; cursor:default; }
      .aagmg-log { height:320px; width:100%; box-sizing:border-box; overflow-y:auto;
                 background:#141414; border:1px solid #333; border-radius:4px; padding:8px;
                 display:flex; flex-direction:column; gap:6px; }
      .aagmg-log[hidden] { display:none; }
      .aagmg-msg { white-space:pre-wrap; word-break:break-word; padding:6px 8px;
                 border-radius:6px; font-size:13px; line-height:1.4; }
      .aagmg-msg .aagmg-who { display:block; font-size:11px; opacity:0.75; margin-bottom:2px; }
      .aagmg-user { background:#22323a; border-left:3px solid #00ffcc; align-self:flex-end; max-width:85%; }
      .aagmg-assistant { background:#2a2a2a; border-left:3px solid #ffaa00; align-self:flex-start; max-width:85%; }
      .aagmg-sys { color:#ffaa00; font-size:12px; align-self:center; }
      .aagmg-input { display:block; width:100% !important; box-sizing:border-box !important;
      ...
      .aagmg-input::placeholder { color:#888; }
      .aagmg-input.aagmg-drop { border-color:#ffaa00; box-shadow:0 0 6px rgba(255,170,0,0.4); }
      .aagmg-send { display:block; width:100%; box-sizing:border-box; padding:9px 0;
                  background:#00ffcc; color:#0a0a0a; border:none; border-radius:4px;
                  cursor:pointer; font-weight:600; font-size:14px; }
      .aagmg-send:hover { background:#33ffd6; }
      .aagmg-rollback { align-self:stretch; max-width:100%; background:#241f12;
                     border:1px solid #ffaa00; border-radius:6px; padding:8px; }
      .aagmg-cf-h { color:#ffaa00; font-weight:600; font-size:12px; margin-bottom:4px; }
      .aagmg-cf-sum { font-size:13px; margin-bottom:6px; white-space:pre-wrap; }
      .aagmg-cf-row { display:flex; gap:8px; margin-top:8px; }
      .aagmg-cf-btn { flex:1; padding:7px 0; border:none; border-radius:4px;
                    cursor:pointer; font-weight:600; }
      .aagmg-cf-action { background:#00ffcc; color:#0a0a0a; }
      .aagmg-cf-action:hover { background:#33ffd6; }
      .aagmg-cf-action:disabled { opacity:0.5; cursor:default; }
      .aagmg-rp-cap { font-size:11px; opacity:0.75; margin-bottom:4px; }
      .aagmg-rp-docs { font-size:11px; opacity:0.85; }
      .aagmg-rp-docs summary { cursor:pointer; }
      .aagmg-rp-docs ul { margin:2px 0 0 16px; padding:0; }
      .aagmg-rp-done { font-size:12px; color:#ffaa00; margin-top:6px; }
    `;
  ...
  const content = `
    <div class="forge-dialog-dark aagmg-wrap" data-aagmg="wrap">
      <div class="aagmg-status warn" data-aagmg="status">${L('StatusNoListener')}</div>
      <div class="aagmg-tabs" data-aagmg="tabs"${TABS_ON ? '' : ' style="display:none"'}></div>
      <div class="aagmg-logs" data-aagmg="logs"></div>
      <textarea class="aagmg-input" data-aagmg="input" placeholder="${L('Placeholder')}"></textarea>
      <button type="button" class="aagmg-send" data-aagmg="send">${L('Send')}</button>
    </div>`;

  let root = null;
  const $el = (k) => root?.querySelector(`[data-aagmg="${k}"]`);
  const tabs = new Map();
  let activeId = null;
  ...
  const renderBar = () => {
    ...
    for (const tab of tabs.values()) {
      ...
      button.type = 'button';
      button.className = 'aagmg-tab aagmg-' + tab.state + (tab.id === activeId ? ' aagmg-active' : '');
      button.dataset.tab = tab.id;
      const title = document.createElement('span');
      title.className = 'aagmg-tab-t';
      title.textContent = tab.title;
      title.title = tab.title;
      button.appendChild(title);
      const close = document.createElement('span');
      close.className = 'aagmg-tab-x';
    ...
    add.type = 'button';
    add.className = 'aagmg-tab-add';
    add.dataset.aagmg = 'tabadd';
    add.textContent = '+';
    add.title = L('TabAdd');
  ...
  const addMsg = (role, text, tabId) => {
    ...
    msg.className = 'aagmg-msg aagmg-' + role;
    if (role === 'user' || role === 'assistant') {
      const who = document.createElement('span');
      who.className = 'aagmg-who';
  ...
  let sessionMode = 'internal', lastState = 'no-listener';
  const setStatus = (state, text) => {
    const bar = $el('status');
    if (!bar) return;
    lastState = state || lastState;
    const ready = lastState === 'ready' || lastState === 'working';
    const modeBit = sessionMode === 'external' ? L('ModeExternal') : L('ModeInternal');
    bar.className = 'aagmg-status ' + (ready ? 'ready' : 'warn');
    bar.textContent = modeBit + ' ' + (text || (lastState === 'working' ? L('StatusWorking')
      : ready ? L('StatusReady')
      : lastState === 'disconnected' ? L('StatusDisconnected')
      : L('StatusNoListener')));
  };
  ...
  const markRolled = (card, state) => {
    card.querySelector('.aagmg-cf-action')?.remove();
    card.querySelector('.aagmg-rp-done')?.remove();
    card.appendChild(div('aagmg-rp-done', state === 'partial' ? L('RollbackPartial') : L('RollbackApplied')));
  };
  ...
  const renderPoint = (pt) => {
    ...
    card.className = 'aagmg-msg aagmg-rollback';
    card.dataset.rollback = pt.id;
    card.appendChild(div('aagmg-cf-h', `${L('RollbackPoint')} ${pt.id} (${pt.kind || 'eval'})`));
    card.appendChild(div('aagmg-cf-sum', pt.summary || 'Write'));
    if (pt.captured) card.appendChild(div('aagmg-rp-cap', pt.captured));
    const docs = Array.isArray(pt.docs) ? pt.docs : [];
    if (docs.length) {
      const det = document.createElement('details');
      det.className = 'aagmg-rp-docs';
    ...
    if (!pt.state || pt.state === 'live') {
      ...
      action.type = 'button';
      action.className = 'aagmg-cf-btn aagmg-cf-action';
  ...
  const submit = () => {
    ...
    if (tab.state === 'working') addMsg('sys', L('Interrupt'), tab.id);
  ...
  const unsubStatus = api.onStatus((p) => {
    if (p?.mode === 'external' || p?.mode === 'internal') sessionMode = p.mode;
    setStatus(p?.state || 'no-listener', p?.text);
  });
  const unsubTabs = api.onTabs?.((p) => syncTabs(p?.tabs));
  const unsubMode = api.onMode?.((p) => {
    sessionMode = p?.mode === 'external' ? 'external' : 'internal';
    setStatus(lastState);
  });
  ...
  const dlg = new Dialog({
    ...
    render: (html) => {
      ...
      $el('tabs')?.addEventListener('click', (ev) => {
        ...
        if (ev.target.closest?.('[data-aagmg="tabadd"]')) { addTab(); return; }
      ...
      const wrap = $el('wrap');
      let dragDepth = 0;
      const undrop = () => { dragDepth = 0; $el('input')?.classList.remove('aagmg-drop'); };
      wrap?.addEventListener('dragenter', (ev) => {
        ev.preventDefault();
        dragDepth++;
        $el('input')?.classList.add('aagmg-drop');
      });
  ...
  }, { width: 560, resizable: false, classes: ['aagmg-dialog'] });

  dlg.render(true);
}

export const CHAT_MACRO_COMMAND =
  `(${chatBoxMain.toString()})().catch((e) => console.error('[aagm-g] chat macro error:', e));`;
