...
import { truncateFields, truncateForLog } from './audit.js';
...
export async function startMcpServer({
  ...
  const makeServer = () => {
    const s = new McpServer({ name: 'aagm-g-relay', version: '2.0.0' });
...
async function handlePost(req, res, makeServer, audit, claimPromptListener) {
  ...
  req.on('end', async () => {
    ...
    if (parsed?.method === 'tools/call') {
      audit.log('mcp.call', { name: parsed.params?.name, arguments: truncateFields(parsed.params?.arguments || {}) });
    }
    if (!claimPromptListener(parsed)) {
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(JSON.stringify({
        jsonrpc: '2.0',
        error: { code: -33005, message: 'listener-occupied: another AAGM-G listener is already active' },
        id: parsed?.id ?? null,
      }));
      return;
    }
...
function registerTools(server, dispatcher, audit, promptQueue, capabilitySet, worldSettings, mirror, writeQueue, rollbacks, tabs) {
  const callBridge = (method, params, timeoutMs) => dispatcher.sendToBridge({
    capabilitySet, method, params, ...(timeoutMs ? { timeoutMs } : {}),
  });
  ...
  const steer = (tabId) => {
    const prompts = promptQueue.takeInterrupt(tabId || null);
    if (!prompts.length) return null;
    audit.log('chat.interrupt', { count: prompts.length });
    return asText({
      interrupted: true,
      prompts,
      instruction: 'Stop the current task. Do not continue it. Handle these prompts now.',
    });
  };
  const run = async (tabId, task) => {
    const hit = steer(tabId);
    if (hit) return hit;
    promptQueue.markBusy(true);
    try { return await task(); }
    finally { promptQueue.markBusy(false); }
  };

  server.tool(
    'foundry_ping',
    'Liveness check across the AAGM-G bridge. Returns pong, Foundry server time, the current ' +
    'mode, and the relay-enforced world settings. Use this first to verify the bridge and read posture.',
    {},
    async () => run(null, async () => {
  ...
  server.tool(
    ...
    async ({ actorId, fields }) => run(null, async () => asText(await callBridge('query.actor', { actorId, fields })))
  );
  ...
  server.tool(
    ...
    async ({ sceneId }) => run(null, async () => asText(await callBridge('query.scene', { sceneId })))
  );
  ...
  server.tool(
    ...
    async ({ macroId, name }) => run(null, async () => asText(await callBridge('query.macro', { macroId, name })))
  );

  server.tool(
    'foundry_query_journal',
    'Read journal entry / page content. Provide `journalId` or `name` to identify the entry, and optionally `pageId` or `pageName` to target a specific page. Protected database journals return blocked.',
    {
    ...
    async (params) => run(null, async () => asText(await callBridge('query.journal', params)))
  );
  ...
  server.tool(
    ...
    async ({ userId }) => run(null, async () => asText(await callBridge('query.user', { userId })))
  );
  ...
  server.tool(
    ...
    async ({ durationSeconds, levels, filter }, extra) => run(null, async () => {
      const collected = [];
  ...
  server.tool(
    ...
    async ({ code, intent, summary, awaitResult, captureConsole, tabId }) => run(tabId, async () => {
  ...
  server.tool(
    ...
    async ({ targets, amount, summary, note, tabId }) => run(tabId, async () => {
  ...
  server.tool(
    ...
    'macros. Existing files rotate to .bkp and no file is deleted. Reads run immediately.',
    {
      name: z.string().optional().describe('Exact world macro name. Omit for all macros.'),
    },
    async ({ name }) => run(null, async () => {
      try {
  ...
  server.tool(
    ...
    'before a bulk restore. Restore one macro per foundry_mirror_restore call.',
    {},
    async () => run(null, async () => {
  ...
  server.tool(
    ...
    async ({ name, tabId }) => run(tabId, async () => {
      let record;
      ...
      audit.log('mirror.restore', { name: record.name, created: !!result?.created, rollbackId: result?.rollbackPoint?.id || null });
  ...
  server.tool(
    ...
    {},
    async () => run(null, async () => asText({ points: rollbacks.list() }))
  );

  server.tool(
    'foundry_session_logs',
    'List local AAGM-G Markdown session logs newest first.',
    {},
    async () => run(null, async () => asText({ logs: audit.listLogs() }))
  );

  server.tool(
    'foundry_read_session_log',
    'Read one local AAGM-G Markdown session log. Omit `name` for the current log.',
    { name: z.string().optional().describe('Exact .md filename returned by foundry_session_logs') },
    async ({ name }) => run(null, async () => asText({ name: name || null, text: audit.readLog(name) }))
  );
  ...
  server.tool(
    ...
    async ({ rollbackId, tabId }) => run(tabId, async () => writeQueue.run('rollback', async () =>
      asText(await rollbacks.rollbackTo(rollbackId, { via: 'tool' }))))
  );
  ...
  server.tool(
    'foundry_get_prompts',
    'Long-polling drain of chat messages the GM typed in the in-Foundry "Open AAGM-G Chat" ' +
    ...
    async ({ listenerId }) => {
      if (!promptQueue.claimListener(listenerId)) {
        audit.log('chat.refused', { listenerId });
        dispatcher.notifyBridge({ capabilitySet, method: 'aagm.listener.refused', params: {} });
        const error = new Error('listener-occupied: another AAGM-G listener is already draining this chat box');
        error.code = -33005;
        throw error;
      }
      ...
      r.tabs = tabs.list();
      r.closedTabs = r.hold ? [] : tabs.drainClosed();
  ...
  server.tool(
    ...
    async ({ listenerId, tabId }) => {
      if (!promptQueue.claimListener(listenerId)) {
        const error = new Error('listener-occupied: another AAGM-G listener owns this chat box');
        error.code = -33005;
        throw error;
      }
  ...
  server.tool(
    ...
    'Set whether the GM is working inside the Foundry chat (/int) or with Grok outside it (/ext). ' +
    'Pass internal or external. /int is the default.',
  ...
  server.tool(
    ...
    'Read the AAGM-G Loot Watchdog rescue queue. Returns pending real items, report-only ' +
    ...
    {},
    async () => run(null, async () => asText(await callBridge('loot.pending', {})))
  );

  server.tool(
    'foundry_restore_loot',
    'Restore vanished loot recorded by the AAGM-G Loot Watchdog. This constrained primitive ' +
    ...
    async ({ eventIds, ackPhantoms }) => run(null, async () => {
      const result = await rollbackWrite({ label: 'Restore Loot Watchdog records', kind: 'loot.restore' },
