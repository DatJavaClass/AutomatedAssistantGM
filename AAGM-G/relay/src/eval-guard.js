/* Eval classifier. Rollback coverage remains authoritative. */
...
const NONROLLBACKABLE = [
  /\bChatMessage\s*\.\s*create(?:Documents)?\s*\(/,
...
/* Deletes require a complete rollback point. */
const DESTRUCTIVE = [
...
/* Other writes require a complete rollback point. */
const MUTATING = [
...
export function classifyEval(code) {
  ...
  if ((m = firstMatch(c, NONROLLBACKABLE))) return { category: 'side-effect', ...m };
...
/* Higher values win. */
export const SEVERITY = { read: 0, mutating: 1, destructive: 2, 'side-effect': 3, 'db-journal': 4 };
