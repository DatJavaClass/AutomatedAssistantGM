...
async function lootWatchdogMain() {
  ...
  const FLAG_KEY = 'aagm-k-loot-rescue';
  const WINDOW_KEY = '__aagmOLootWatchdog';
  const VERIFY_WINDOW_MS = 4000;
  const PAGE_NAME = 'Session Log';
  const TAG = '[AAGM-KLootWatchdog]';
  const L = (k) => game.i18n.localize('AAGM_K.LOOT.' + k);
  ...
  async function settle(rec) {
    ...
    if (rec.kind === 'phantom') {
      ...
      await ChatMessage.create({
        ...
        speaker: { alias: 'AAGM-K Loot Watchdog' },
      });
    ...
    await ChatMessage.create({
      content: '<div style="border:2px solid #c00; padding:8px; background:#fee;">'
        ...
        + '<em>Queued for AAGM-K auto-restore. Event ' + esc(rec.eventId) + '</em></div>',
      whisper: ChatMessage.getWhisperRecipients('GM'),
      speaker: { alias: 'AAGM-K Loot Watchdog' },
    });
...
export const LOOT_MACRO_COMMAND =
  `(${lootWatchdogMain.toString()})().catch((e) => console.error('[aagm-k] loot watchdog macro error:', e));`;
