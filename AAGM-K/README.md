...
- Non interactive: `kimi -p "<listener prompt>"`. It runs one prompt and, in
  our testing, edits without asking; it cannot combine with `-y` or
  `--auto`.
- Interactive: run `kimi`, then paste the prompt from
  [`relay/LISTENER_PROMPT.md`](relay/LISTENER_PROMPT.md).

AAGM-K intentionally remains localhost only, so hosted or remote Kimi use is
outside this build.
...
1. Enable the AAGM-K bridge in one GM tab.
2. Open **Configure Settings > AAGM-K Settings** and select a mode.
3. Leave the bridge disabled in every other tab using the same GM account.
4. Run the auto-created **Open AAGM-K Chat** macro.
5. Run **AAGM-K Loot Watchdog** once to arm it when Item Piles rescue is needed.
