...
export async function startMcpServer({
  ...
  const makeServer = () => {
    const s = new McpServer({ name: 'aagm-k-relay', version: '2.0.0' });
  ...
  req.on('end', async () => {
    ...
    if (!claimPromptListener(parsed)) {
      ...
      res.end(JSON.stringify({
        jsonrpc: '2.0',
        error: { code: -33005, message: 'listener-occupied: another AAGM-K listener is already active' },
        id: parsed?.id ?? null,
      }));
      return;
    }
...
function registerTools(server, dispatcher, audit, promptQueue, capabilitySet, worldSettings, mirror, writeQueue, rollbacks, tabs) {
  ...
  server.tool(
    'foundry_ping',
    'Liveness check across the AAGM-K bridge. Returns pong, Foundry server time, the current ' +
  ...
  server.tool(
    ...
    'List local AAGM-K Markdown session logs newest first.',
    {},
  ...
  server.tool(
    ...
    'Read one local AAGM-K Markdown session log. Omit `name` for the current log.',
  ...
  server.tool(
    'foundry_get_prompts',
    'Long-polling drain of chat messages the GM typed in the in-Foundry "Open AAGM-K Chat" ' +
    ...
    async ({ listenerId }) => {
      if (!promptQueue.claimListener(listenerId)) {
        audit.log('chat.refused', { listenerId });
        dispatcher.notifyBridge({ capabilitySet, method: 'aagm.listener.refused', params: {} });
        const error = new Error('listener-occupied: another AAGM-K listener is already draining this chat box');
        error.code = -33005;
        throw error;
      }
  ...
  server.tool(
    ...
    async ({ listenerId, tabId }) => {
      if (!promptQueue.claimListener(listenerId)) {
        const error = new Error('listener-occupied: another AAGM-K listener owns this chat box');
        error.code = -33005;
        throw error;
      }
  ...
  server.tool(
    ...
    'Set whether the GM is working inside the Foundry chat or with an external Kimi session. ' +
  ...
  server.tool(
    ...
    'Read the AAGM-K Loot Watchdog rescue queue. Returns pending real items, report-only ' +
  ...
  server.tool(
    ...
    'Restore vanished loot recorded by the AAGM-K Loot Watchdog. This constrained primitive ' +
